...
const ToneListControl = ({ toneList, favourites, onApplyTone, onEditTone, noneMsg, enableToneEditor }) => {
        ...
        return toneList.map((tone: Tone) => (
            <div key={tone.toneId} className="tone">
                <div className="row">
                    ...
                    <div className="col-md-4">
                        <span className="badge rounded-pill bg-primary">{mapDeviceType(tone.deviceType)}</span>
    ...
    return (
        <div>{!toneList || toneList.length == 0 ? <div>{noResultsMessage}</div> : <div>{renderToneList()}</div>}</div>
    );
};
