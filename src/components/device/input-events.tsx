...
const InputEventsControl = () => {
    const inputEventMappings: InputEventMapping[] = AppStateStore.useState((s) => s.inputEventMappings);

    const selectedMidiInput: string = AppStateStore.useState((s) => s.selectedMidiInput);
        ...
        try {
            ...
            Utils.sleepAsync(1000).then(() => {
                if (AppStateStore.getRawState().selectedMidiInput != null) {
                    console.log("Selecting input " + AppStateStore.getRawState().selectedMidiInput);
    ...
    const setupMidiEvents = (midiDeviceName?: string) => {
        if (midiAvailable) {
                                ...
                                try {
                                    if (
                                        ...
                                        deviceViewModel
                                            ...
                                            .catch((err) => {
                                                console.log(
                                                    "Failed to set channel. Device may not be connected " + err
                                                );
                                            })
                                            .then(() => {
                                                console.log(
                                                    "Midi input event channel selection:" + mapping.target.value
                                                );
                                            });
                                    }
                                } catch (err) {
            ...
            console.log("Midi/input events changed but midi not yet available : " + midiDeviceName);
        }
    };
