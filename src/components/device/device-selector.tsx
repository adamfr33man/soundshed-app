...
const DeviceSelectorControl = () => {
    const connectionInProgress = DeviceStateStore.useState((s) => s.isConnectionInProgress);
    ...
    const deviceScanInProgress = DeviceStateStore.useState((s) => s.isDeviceScanInProgress);
    const deviceScanFailed = DeviceStateStore.useState((s) => s.deviceScanFailed);
    const deviceConnectionFailed = DeviceStateStore.useState((s) => s.deviceConnectionFailed);
    ...
    useEffect(() => {
        if (!connected) {
            ...
            if (lastConnectedDevice) {
                console.info("Re-connecting last known device [" + lastConnectedDevice.name + "]");
    ...
    const listItems = (l: BluetoothDeviceInfo[]) => {
        let list = l.map((i) =>
            attemptedDevice != null && attemptedDevice.address == i.address ? attemptedDevice : i
        );

        if (deviceScanFailed) {
            return (
                <div className="alert alert-warning">
                    Could not scan for devices or scan was cancelled. Bluetooth may not be switched on or your
                    browser/OS may not support Bluetooth.
                </div>
            );
        }
        ...
        if (deviceConnectionFailed) {
            return (
                <div className="alert alert-danger">
                    Failed to connect to device. Try restarting the device, restarting your computer or unpair the
            ...
            return list.map((d) => (
                <div key={d.address.toString()} className="row m-2">
                    <div className="col-md-6">
                        <span className="badge rounded-pill bg-secondary">{d.name}</span> ({d.address})
                        {d.connectionFailed == true ? (
                            <p className="text-warning">
                                Connection failed. Check device is on and paired with this computer.
                            </p>
                        ) : (
                            ""
                        )}
    ...
    return (
        <div className="amp-intro">
            ...
            <p>
                To get started, switch on your amp, pair it via Bluetooth (Spark 40 BLE or Spark Mini) and then select
                Scan to find your device. You should ensure that the amp is not already connected to a different device
                as the amp can only connect to one device at a time.
            </p>
            ...
            {deviceScanInProgress ? (
                ...
                <button type="button" className="btn btn-sm btn-primary" onClick={requestScanForDevices}>
                    Scan
                </button>
            )}
