...
const FxControl = ({ fx, onFxParamChange, onFxToggle }) => {
    ...
    React.useEffect(() => {
        ...
        if (fxDefinition) {
            ...
            listOfSameTypeFx = listOfSameTypeFx.sort((a, b) => {
    ...
    const handleFxChange = (e) => {
        deviceViewModel.requestFxChange({ dspIdOld: fx.type, dspIdNew: e.target.value }).then((changedOk) => {
            if (!changedOk) {
                alert("Fx change failed. FX type may have been removed, or amp not connected");
            }
        });
    };
        ...
        <div className="fx">
            ...
            <div>
                ...
                {isExperimentalFxSelected ? (
                    <span className="badge rounded-pill bg-danger m-1" title="Experimental FX - may not work at all.">
                        Experimental FX
                    </span>
                ) : (
                    ""
