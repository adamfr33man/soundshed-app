            ...
            return t.fx.map((fx) =>
                fx.type != "pg.spark40." ? (
                    <td key={fx.type.toString()}>
                        <FxControl fx={fx} onFxParamChange={onFxParamChange} onFxToggle={onFxToggle}></FxControl>
                    </td>
                ) : (
                    ""
                )
        ...
        <div>
            {!signalPathState || !signalPathState.fx || signalPathState.fx.length == 0 ? (
                <div className="container">
                    <label>
                        No preset selected (or amp not connected). Connect and refresh to get current amp settings. You
                        may need to select a preset button on the amp to start.
