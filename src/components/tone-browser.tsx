...
const ToneBrowserControl = () => {
    ...
    const enableCommunityTones = UIFeatureToggleStore.useState((s) => s.enableCommunityTones);

    const enableToneCloud = UIFeatureToggleStore.useState((s) => s.enabledPGToneCloud);

    const enableToneEditor = UIFeatureToggleStore.useState((s) => s.enableToneEditor);
    ...
    const onApplyTone = async (tone) => {
        ...
        if (t.schemaVersion == "pg.preset.summary" && t.fx == null) {
            ...
            let result = await appViewModel.loadToneCloudPreset(t.toneId.replace("pg.tc.", ""));
    ...
    return (
        <div className="tones-intro">
            ...
            <div className="info">
                <Nav variant="tabs" activeKey={viewSelection} onSelect={(selectedKey) => setViewSelection(selectedKey)}>
                    {enableMyTones ? (
                        <Nav.Item>
