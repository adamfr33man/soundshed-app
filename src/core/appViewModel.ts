...
export class AppViewModel {
    ...
    init() {
        if (this.soundshedApi.isUserSignedIn()) {
            AppStateStore.update((s) => {
                s.isUserSignedIn = true;
                s.userInfo = this.soundshedApi.getCurrentUserInfo();
            });
        } else {
            AppStateStore.update((s) => {
                s.isUserSignedIn = false;
                s.userInfo = null;
            });
        }
    }
        ...
        let mappings: InputEventMapping[] = [
            {
                id: "1",
                name: "C3 to CH1",
                source: { type: "midi", code: "48", channel: "1" },
                target: { type: "amp-channel", value: "0" },
            },
            {
                id: "2",
                name: "D3 to CH2",
                source: { type: "midi", code: "50", channel: "1" },
                target: { type: "amp-channel", value: "1" },
            },
            {
                id: "3",
                name: "E3 to CH3",
                source: { type: "midi", code: "52", channel: "1" },
                target: { type: "amp-channel", value: "2" },
            },
            {
                id: "4",
                name: "F3 to CH4",
                source: { type: "midi", code: "53", channel: "1" },
                target: { type: "amp-channel", value: "3" },
            },
            {
                id: "5",
                name: "Key 1 to CH1",
                source: { type: "keyboard", code: "1" },
                target: { type: "amp-channel", value: "0" },
            },
            {
                id: "6",
                name: "Key 2 to CH1",
                source: { type: "keyboard", code: "2" },
                target: { type: "amp-channel", value: "1" },
            },
            {
                id: "7",
                name: "Key 3 to CH1",
                source: { type: "keyboard", code: "3" },
                target: { type: "amp-channel", value: "2" },
            },
            {
                id: "8",
                name: "Key 4 to CH1",
                source: { type: "keyboard", code: "4" },
                target: { type: "amp-channel", value: "3" },
            },
        ];
        ...
        if (preset != null) {
            ...
            if (favourites.find((t) => t.name.toLowerCase() == convertedTone.name.toLowerCase())) {
                if (
                    autoOverwrite ||
                    confirm(
                        "You already have a preset stored with the same name. Do you wish to overwrite it with this one?"
                    )
                ) {
                    // update existing
            ...
            if (
                favourites.find(
                    (t) =>
                        t.name.toLowerCase() != convertedTone.name.toLowerCase() &&
                        t.toneId.toLowerCase() == convertedTone.toneId.toLowerCase()
                )
            ) {
                // update existing
    ...
    async loadLatestTones(): Promise<Tone[]> {
        try {
            TonesStateStore.update((s) => {
                s.isSearchInProgress = true;
            });
            const result = await this.soundshedApi.getTones();

            TonesStateStore.update((s) => {
                s.toneResults = result.result ?? [];
            });
        ...
        try {
            if (preferCached) {
                ...
                if (cached != null) {
                    ...
                    TonesStateStore.update((s) => {
                        s.toneCloudResults = cachedTones;
                    });
