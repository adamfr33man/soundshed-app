...
`;
                    ...
                    if (window.webAudioControlsWidgetManager) window.webAudioControlsWidgetManager.addWidget(this);
                }
...
<defs>
  ...
  <line id="K" x1="32" y1="25" x2="32" y2="11" stroke-linecap="round" stroke-width="6" stroke="${this.coltab[0]}"/>
                        ...
                        for (let i = 0; i < 101; ++i) {
                            svg += `<use href="#B" y="${64 * i}"/><use href="#K" y="${64 * i}" transform="rotate(${(
                                -135 +
                                (270 * i) / 101
                            ).toFixed(2)},32,${64 * i + 32})"/>`;
                        }
                        svg += "</svg>";
                        ...
                        this.img.onload = () => {
                            ...
                            else this.elem.style.backgroundSize = `${this.kw}px ${this.kh * (this.sprites + 1)}px`;
                            this.elem.style.width = this.kw + "px";
                            this.elem.style.height = this.kh + "px";
                            this.style.height = this.kh + "px";
                            this.redraw();
...
`;
                ...
                setupImage() {
                    ...
                    if (this.src == null || this.src == "") {
                        ...
                        const svgbody = `<svg xmlns="http://www.w3.org/2000/svg" width="${this.sw}" height="${
                            this.sh
...
<rect x="3" y="3" rx="${r}" ry="${r}" width="${this.sw - 6}" height="${this.sh - 6}" fill="${
                            this.coltab[1]
                        }" filter="url(#f1)"/>
<rect x="1" y="1" rx="${r}" ry="${r}" width="${this.sw - 2}" height="${this.sh - 2}" fill="url(#g1)"/>
                    ...
                    this.bodyimg.onload = () => {
                        ...
                        if (this._knobsrc == null) {
                            ...
                            const svgknob = `<svg xmlns="http://www.w3.org/2000/svg" width="${this.kw}" height="${
                                this.kh
                            }" preserveAspectRatio="none">
<defs>
  <filter id="f1">
                ...
                pointerdown(ev) {
                    ...
                    let pointermove = (ev) => {
                        ...
                        if (this.tracking == "abs") {
                            ...
                            } else this._setValue(this.min + (this.max - this.min) * val);
                        } else {
                            let offset =
                                ((this.startPosY - e.pageY) * this.sensey - (this.startPosX - e.pageX) * this.sensex) *
                                this.sensitivity;
                            if (this.log) {
                                let r = Math.log(this.startVal / this.min) / Math.log(this.max / this.min);
                                r += offset / ((e.shiftKey ? 4 : 1) * 128);
...
`;
                ...
                setupImage() {
                    ...
                    if (this.src == null || this.src == "") {
                        ...
                        const kw = this.kw,
                            kh = this.kh;
                        const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${this.kw}" height="${
                            this.kh * 2
                        }" preserveAspectRatio="none">
<defs>
<linearGradient id="g1" x1="0%" y1="0%" x2="0%" y2="100%">
...
<g id="p1">
  <rect x="${kw * 0.075}" y="${kh * 0.075}" width="${kw * 0.85}" height="${kh * 0.85}" rx="${mm * 0.1}" ry="${
      mm * 0.1
  }" fill="#000"/>
  <rect x="${kw * 0.1}" y="${kh * 0.1}" width="${kw * 0.8}" height="${kh * 0.8}" rx="${mm * 0.1}" ry="${
      mm * 0.1
  }" fill="${this.coltab[1]}"/>
</g>
<g id="p2">
  <circle cx="${kw * 0.5}" cy="${kh * 0.5}" r="${mm * 0.35}" stroke="#000" stroke-width="${mm * 0.03}" fill="${
      this.coltab[0]
  }" filter="url(#f1)"/>
  <circle cx="${kw * 0.5}" cy="${kh * 0.5}" r="${mm * 0.27}" stroke="#000" stroke-width="${
      mm * 0.03
  }" fill="#000" filter="url(#f1)"/>
  <rect x="${kw * 0.075}" y="${kh * 0.075}" width="${kw * 0.85}" height="${kh * 0.85}" rx="${mm * 0.1}" ry="${
      mm * 0.1
  }" fill="url(#g1)"/>
</g>
                ...
                setupImage() {
                    this.imgloaded = () => {
                        ...
                        if (l && typeof l.value != "undefined") {
                            ...
                            if (this.currentLink) this.currentLink.removeEventListener("input", this.currentLink.func);
                            this.currentLink = {
                                target: l,
                                func: (e) => {
                                    if (typeof l.convValue == "number") this.setValue(l.convValue.toFixed(l.digits));
